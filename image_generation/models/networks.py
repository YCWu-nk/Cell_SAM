...
class TransformerConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0):
        super(DynamicSparseTransformerConv2d, self).__init__()
        ...
        self.embedding = nn.Linear(self.in_channels, self.embedding_dim)
        ...
        self.linear = nn.Linear(self.embedding_dim, self.out_channels)
    ...
    def forward(self, x):
        ...
        encoded = encoded.permute(1, 2, 0).view(batch_size, -1, height, width)
...
class StridedConvF(nn.Module):
    def __init__(self, init_type='normal', init_gain=0.02, gpu_ids=[]):
        super().__init__()
        # self.conv1 = TransformerConv2d(256, 128, 3, stride=2)
        # self.conv2 = TransformerConv2d(128, 64, 3, stride=1)
    ...
    def create_mlp(self, x):
        ...
        for i in range(n_down):
            mlp.append(TransformerConv2d(C, max(C // 2, 64), 3, stride=2))
            mlp.append(nn.ReLU())
            C = max(C // 2, 64)
        mlp.append(TransformerConv2d(C, 64, 3))
...
class StyleEncoder(nn.Module):
    def __init__(self, n_downsample, input_dim, dim, style_dim, norm, activ, vae=False):
        ...
        if self.vae:
            ...
            self.model += [TransformerConv2d(dim, style_dim, 1, 1, 0)]
...
class Conv2dBlock(nn.Module):
    def __init__(self, input_dim, output_dim, kernel_size, stride,
        ...
        self.conv = TransformerConv2d(input_dim, output_dim, kernel_size, stride, bias=self.use_bias)
...
class ResnetGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, padding_type='reflect', no_antialias=False, no_antialias_up=False, opt=None):
        ...
        model = [nn.ReflectionPad2d(3),
                 TransformerConv2d(input_nc, ngf, kernel_size=7, padding=0, bias=use_bias),
                 norm_layer(ngf),
                 nn.ReLU(True)]
        ...
        for i in range(n_downsampling):  # add downsampling layers
            mult = 2 ** i
            if(no_antialias):
                model += [TransformerConv2d(ngf * mult, ngf * mult * 2, kernel_size=3, stride=2, padding=1, bias=use_bias),
                          norm_layer(ngf * mult * 2),
                          nn.ReLU(True)]
            else:
                model += [TransformerConv2d(ngf * mult, ngf * mult * 2, kernel_size=3, stride=1, padding=1, bias=use_bias),
                          norm_layer(ngf * mult * 2),
                          nn.ReLU(True),
        ...
        for i in range(n_downsampling):  # add upsampling layers
            ...
            if no_antialias_up:
                ...
                model += [Upsample(ngf * mult),
                          TransformerConv2d(ngf * mult, int(ngf * mult / 2),
                          ...
                          nn.ReLU(True)]
        model += [nn.ReflectionPad2d(3)]
        model += [TransformerConv2d(ngf, output_nc, kernel_size=7, padding=0)]
        model += [nn.Tanh()]
...
class ResnetBlock(nn.Module):
    ...
    def build_conv_block(self, dim, padding_type, norm_layer, use_dropout, use_bias):
        ...
        conv_block += [TransformerConv2d(dim, dim, kernel_size=3, padding=p, bias=use_bias), norm_layer(dim), nn.ReLU(True)]
        if use_dropout:
        ...
        if padding_type == 'reflect':
            ...
            raise NotImplementedError('padding [%s] is not implemented' % padding_type)
        conv_block += [TransformerConv2d(dim, dim, kernel_size=3, padding=p, bias=use_bias), norm_layer(dim)]
...
class UnetSkipConnectionBlock(nn.Module):
    ...
    def __init__(self, outer_nc, inner_nc, input_nc=None,
        ...
        if input_nc is None:
            input_nc = outer_nc
        downconv = TransformerConv2d(input_nc, inner_nc, kernel_size=4,
...
class NLayerDiscriminator(nn.Module):
    ...
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d, no_antialias=False):
        ...
        padw = 1
        if(no_antialias):
            sequence = [TransformerConv2d(input_nc, ndf, kernel_size=kw, stride=2, padding=padw), nn.LeakyReLU(0.2, True)]
        else:
            sequence = [TransformerConv2d(input_nc, ndf, kernel_size=kw, stride=1, padding=padw), nn.LeakyReLU(0.2, True), Downsample(ndf)]
        nf_mult = 1
        nf_mult_prev = 1
        for n in range(1, n_layers):  # gradually increase the number of filters
            ...
            if(no_antialias):
                sequence += [
                    TransformerConv2d(ndf * nf_mult_prev, ndf * nf_mult, kernel_size=kw, stride=2, padding=padw, bias=use_bias),
                    norm_layer(ndf * nf_mult),
                    nn.LeakyReLU(0.2, True)
                ]
            else:
                sequence += [
                    TransformerConv2d(ndf * nf_mult_prev, ndf * nf_mult, kernel_size=kw, stride=1, padding=padw, bias=use_bias),
                    norm_layer(ndf * nf_mult),
                    nn.LeakyReLU(0.2, True),
        ...
        sequence += [TransformerConv2d(ndf * nf_mult, 1, kernel_size=kw, stride=1, padding=padw)]  # output 1 channel prediction map
...
class PixelDiscriminator(nn.Module):
    ...
    def __init__(self, input_nc, ndf=64, norm_layer=nn.BatchNorm2d):
        ...
        self.net = [
            TransformerConv2d(input_nc, ndf, kernel_size=1, stride=1, padding=0),
            nn.LeakyReLU(0.2, True),
            TransformerConv2d(ndf, ndf * 2, kernel_size=1, stride=1, padding=0, bias=use_bias),
            norm_layer(ndf * 2),
            nn.LeakyReLU(0.2, True),
            TransformerConv2d(ndf * 2, 1, kernel_size=1, stride=1, padding=0, bias=use_bias)]
